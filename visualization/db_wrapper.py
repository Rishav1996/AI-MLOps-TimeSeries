...
def get_data(data_ing_id, data_dp_id):
    ...
    result_ing = conn.execute(query).fetchall()
    ...
    result_dp = conn.execute(query).fetchall()
...
def get_forecast_data(train_id):
    ...
    data_fcst_id = conn.execute(f"select data_fcst_id from train_history_table where train_id = {train_id}").fetchone()[0]
    actual_data = conn.execute(f"select period, ts_id, value from data_table where data_id = {data_ing_id}").fetchall()
    actual_data = pd.DataFrame(actual_data, columns=['period', 'ts_id', 'raw_history'])
    forecast_data = conn.execute(f"select period, ts_id, value, split_window, split_no + 1 as split_no, m.model_name\n"
